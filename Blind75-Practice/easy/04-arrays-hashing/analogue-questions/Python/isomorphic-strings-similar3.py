...
class Solution:
    def findAndReplacePattern(self, words: List[str], pattern: str) -> List[str]:
        """
        Approach: Use isomorphic check for each word

        Strategy:
        1. For each word, check if it's isomorphic to pattern
        2. Use two hash maps to ensure bijection
        3. Add matching words to result
        """
...
if __name__ == "__main__":
    ...
    # Test case 1
    result1 = solution.findAndReplacePattern(
        ["abc", "deq", "mee", "aqq", "dkd", "ccc"], "abb"
    )
    assert set(result1) == set(["mee", "aqq"])
    print("✓ Test case 1 passed")

    # Test case 2
    result2 = solution.findAndReplacePattern(["a", "b", "c"], "a")
    assert set(result2) == set(["a", "b", "c"])
